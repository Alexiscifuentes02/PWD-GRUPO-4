...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['botonDeEnvio1'])) {
    ...
    $genero = trim($_GET['formularioGenero'] ?? '');
    $deportes = $_GET['deportes'] ?? []; // Array de deportes seleccionados
    ...
    if (!empty($errores)) {
        include '../vista/E6_formulario.php';
    } else {
        $mensaje = ConcatenarInformacion::generarMensaje($nombre, $apellido, $edad, $direccion, $estudios, $genero, $deportes);
        include '../vista/E6_resultado.php';
    }
} else {
    include '../vista/E6_formulario.php';
}
